...
public class Assembler extends Module {
	...
	public Assembler(Ship _container) {
		super(_container);
		id = Module.ASSEMBLER;
	}

	@Override public int spaceCost() { return 12; }

	@Override public ResourceBundle buildCost() { 
		if (_buildCost == null) {
			_buildCost = new ResourceBundle();
			_buildCost.add(new Resource(Resource.POLYMER_NAME, 92));
	...
	@Override public ResourceBundle demolishReward() {
		if (_demolishReward == null) {
			...
			_demolishReward.add(new Resource(Resource.POLYMER_NAME, 44));
			_demolishReward.add(new Resource(Resource.METAL_NAME, 9));
